...
from keras.layers import Dense, Embedding, Reshape, Input, Conv2D, Flatten, Layer, Lambda, Concatenate
...
class GATLayer(Layer):
    def __init__(self, hidden_dim, **kwargs):
        ...
        self.a = Dense(1, use_bias=False)

    def call(self, inputs):
        # inputs: [node_features, adj_matrix]
        h, adj = inputs  # h: (Batch, 5, features), adj: (Batch, 5, 5)

        # Ensure every node keeps at least a self-loop to avoid fully-masked rows.
        adj = tf.cast(adj, tf.float32)
        n_nodes = tf.shape(adj)[-1]
        batch_size = tf.shape(adj)[0]
        self_loops = tf.eye(n_nodes, batch_shape=[batch_size], dtype=tf.float32)
        adj = tf.maximum(adj, self_loops)

        # 1. 线性变换
        Wh = self.W(h)  # (Batch, 5, hidden_dim)

        # 2. 计算注意力系数 (Self-Attention)
        Wh_i = tf.expand_dims(Wh, 2)  # (Batch, 5, 1, hidden_dim)
        Wh_j = tf.expand_dims(Wh, 1)  # (Batch, 1, 5, hidden_dim)
        a_input = Wh_i + Wh_j         # (Batch, 5, 5, hidden_dim)
        
        e = tf.squeeze(self.a(tf.nn.leaky_relu(a_input)), axis=-1)  # (Batch, 5, 5)

        # 3. 掩码机制 (Masking): 屏蔽不存在的边 (adj中为0的位置)
        mask = -1e9 * (1.0 - adj)
        e_masked = e + mask

        # 4. Softmax 归一化注意力权重
        alpha = tf.nn.softmax(e_masked, axis=-1)

        # 5. 聚合邻居特征
        h_prime = tf.matmul(alpha, Wh)  # (Batch, 5, hidden_dim)
...
class TypedNodeEncoder(Layer):
    def __init__(self, hidden_dim, **kwargs):
        super().__init__(**kwargs)
        self.hidden_dim = hidden_dim

        self.self_encoder = keras.Sequential([
            Dense(hidden_dim, activation='relu'),
            Dense(hidden_dim)
        ], name='self_encoder')

        self.neighbor_encoder = keras.Sequential([
            Dense(hidden_dim, activation='relu'),
            Dense(hidden_dim)
        ], name='neighbor_encoder')
    ...
    def call(self, feat):
        # feat: (B, 5, 6)
        self_feat = feat[:, :1, :]
        neighbor_feat = feat[:, 1:, :]

        self_h = self.self_encoder(self_feat)         # (B, 1, d)
        neighbor_h = self.neighbor_encoder(neighbor_feat)  # (B, 4, d)

        h = tf.concat([self_h, neighbor_h], axis=1)   # (B, 5, d)

        role_ids = tf.constant([0, 1, 2, 3, 4], dtype=tf.int32)
        role_h = self.role_embedding(role_ids)[tf.newaxis, :, :]  # (1, 5, d)
...
homogeneousState = True
NODE_FEATURE_DIM = 18
ROLE_TO_INDEX = {'SELF': 0, 'U': 1, 'D': 2, 'R': 3, 'L': 4}
...
tau         = 0.01      # Soft target update rate (used when hardUpdate=0)
...
nTrain      = 4         # Train every nTrain env steps to reduce correlation in replay samples
...
hardUpdate  = 0         # 0 enables soft update (recommended for GAT stability), 1 uses hard update
updateF     = 400       # Hard-sync frequency (used only when hardUpdate=1)
batchSize   = 64        # Mini-batch size for each gradient update
bufferSize  = 5000      # Replay buffer capacity
replayWarmup= 512       # Minimum replay size before training starts
trainRepeats= 2         # Gradient updates per training trigger
...
class hyperparam:
    def __init__(self, pathing):
        ...
        self.replayWarmup = replayWarmup
        self.trainRepeats = trainRepeats
...
class DDQNAgent:
    def __init__(self, NGT, hyperparams, earth, sat_ID = None):   
        ...
        self.replayWarmup = max(hyperparams.replayWarmup, self.batchS * 4)
        self.trainRepeats = max(1, hyperparams.trainRepeats)
        ...
        if not self.importQ:
            ...
            try:
                ...
                self.qNetwork = keras.models.load_model(
                    nnpath,
                    custom_objects={
                        'GATLayer': GATLayer,
                        'TypedNodeEncoder': TypedNodeEncoder
                    }
                )

                self.qTarget = keras.models.load_model(
                    nnpathTarget,
                    custom_objects={
                        'GATLayer': GATLayer,
                        'TypedNodeEncoder': TypedNodeEncoder
                ...
                # # =========== 添加 custom_objects ===========
                # self.qNetwork = keras.models.load_model(nnpath, custom_objects={'GATLayer': GATLayer})
                # if sat_ID is None:
                #     print('----------------------------------')
                ...
                # if ddqn:
                #     global nnpathTarget
                #     self.qTarget = keras.models.load_model(nnpathTarget, custom_objects={'GATLayer': GATLayer})
                #     # ===========================================    
            # try:
            #     global nnpath
    ...
    def makeDeepAction(self, block, sat, g, earth, prevSat=None):
       ...
       4. Receive reward/penalty
        ...
        if homogeneousState:
            newState = getDeepStateHomogeneous(block, sat, linkedSats)
        elif reducedState:
        ...
        if sat.linkedGT and (block.destination.ID == sat.linkedGT.ID):    # Compare IDs
            ...
            if TrainThis and self.step % nTrain == 0:
                for _ in range(self.trainRepeats):
                    if self.train(sat, earth) == -1:
                        break
            if plotDeliver:
                if int(block.ID[len(block.ID)-1]) == 0: # Draws 1/10 arrivals
                    os.makedirs(earth.outputPath + '/pictures/', exist_ok=True) # drawing delivered
    ...
    def alignQTarget(self, hardUpdate = True): # Soft update is applied on each train step
        '''
    ...
    #     return model
    # def createModel(self):
    #     # 输入 1: 节点特征矩阵 (5个节点, 6个特征维度)
    #     feat_input = Input(shape=(5, 6), name='node_features')
    #     # 输入 2: 邻接矩阵 (5x5)
    #     adj_input = Input(shape=(5, 5), name='adj_matrix')

    #     # 经过两层 GAT 提取图结构特征
    #     h = GATLayer(32)([feat_input, adj_input])
    #     h = GATLayer(32)([h, adj_input])

    #     # 展平图特征（将5个节点的特征拍平）以便送入动作输出层
    #     h_flat = Flatten()(h)

    #     x = Dense(32, activation='relu')(h_flat)
    #     output = Dense(self.actionSize, activation='linear')(x)
    ...
    #     # 1. 先做类型化编码，解决自节点和邻居节点语义不一致
    #     h = TypedNodeEncoder(hidden_dim, name='typed_node_encoder')(feat_input)

    #     # 2. 再做共享图注意力传播
    #     h = GATLayer(hidden_dim, name='gat_1')([h, adj_input])
    #     h = GATLayer(hidden_dim, name='gat_2')([h, adj_input])

    #     # 3. 不再 Flatten 全图，而是按动作构造 4 个中心-邻居对
    #     center = Lambda(lambda x: x[:, :1, :], name='center_node')(h)      # (B, 1, d)
    #     neighbors = Lambda(lambda x: x[:, 1:, :], name='neighbor_nodes')(h) # (B, 4, d)

    #     center_tiled = Lambda(
    #         lambda x: tf.repeat(x, repeats=4, axis=1),
    #         name='center_repeat'
    #     )(center)  # (B, 4, d)

    #     pair_h = Concatenate(axis=-1, name='action_pairs')([center_tiled, neighbors])  # (B, 4, 2d)

    #     x = Dense(hidden_dim, activation='relu', name='action_mlp')(pair_h)
    #     q = Dense(1, activation='linear', name='action_q')(x)  # (B, 4, 1)

    #     output = Lambda(lambda x: tf.squeeze(x, axis=-1), name='q_values')(q)  # (B, 4)
    ...
    def createModel(self):
        ...
        feat_input = Input(shape=(5, NODE_FEATURE_DIM), name='node_features')
        adj_input = Input(shape=(5, 5), name='adj_matrix')

        # 普通 GAT 现在已经成立，因为 5 个节点共享同一套特征语义
        h = GATLayer(hidden_dim, name='gat_1')([feat_input, adj_input])
        h = GATLayer(hidden_dim, name='gat_2')([h, adj_input])

        # 中心节点和 4 个候选邻居分别组成 4 个动作对
        center = Lambda(lambda x: x[:, :1, :], name='center_node')(h)
        neighbors = Lambda(lambda x: x[:, 1:, :], name='neighbor_nodes')(h)
        ...
        pair_h = Concatenate(axis=-1, name='action_pairs')([center_tiled, neighbors])

        x = Dense(32, activation='relu', name='action_mlp_1')(pair_h)
        x = Dense(16, activation='relu', name='action_mlp_2')(x)
        q = Dense(1, activation='linear', name='action_q')(x)

        output = Lambda(lambda x: tf.squeeze(x, axis=-1), name='q_values')(q)
    ...
    def train(self, sat, earth):
        if self.experienceReplay.buffeSize < self.replayWarmup:
            return -1
        ...
        batch_len = actions.shape[0]

        # Actions map to neighbors [U, D, R, L] -> adjacency indices [1, 2, 3, 4].
        next_action_mask = nextStates[1][:, 0, 1:5] > 0
        has_valid_next = np.any(next_action_mask, axis=1)

        # 2. Compute bootstrapped target with action masking.
        next_q_online = self.qNetwork.predict(nextStates, verbose=0)
        if ddqn:
            next_q_target = self.qTarget.predict(nextStates, verbose=0)
            next_q_online_masked = np.where(next_action_mask, next_q_online, -np.inf)
            next_actions = np.argmax(next_q_online_masked, axis=1)
            next_q_target_masked = np.where(next_action_mask, next_q_target, -np.inf)
            next_q = next_q_target_masked[np.arange(batch_len), next_actions]
        else:
            next_q_masked = np.where(next_action_mask, next_q_online, -np.inf)
            next_q = np.max(next_q_masked, axis=1)

        next_q = np.where(has_valid_next, next_q, 0.0)
        next_q = np.nan_to_num(next_q, nan=0.0, posinf=0.0, neginf=0.0)
        expectedRewards = rewards + (1.0 - dones) * self.gamma * next_q

        # 3. Update only selected actions, keep other Q-values unchanged.
        targets = self.qNetwork.predict(states, verbose=0)
        targets[np.arange(batch_len), actions] = expectedRewards
        ...
        # 5. fit the model (states 也是列表)
        loss = self.qNetwork.fit(states, targets, batch_size=self.batchS, epochs=1, verbose=0)
        sat.orbPlane.earth.loss.append([loss.history['loss'][0], sat.env.now])
        earth.trains.append([sat.env.now])

        if ddqn:
            self.alignQTarget(self.hardUpd)
    ...
    def getNextHop(self, newState, linkedSats, sat, block):
        ...
        if explore and random.uniform(0, 1)<self.alignEpsilon(self.step, sat):
            ...
            action   = self.actions[actIndex]
            attempts = 0
            while linkedSats[action] is None and attempts < self.actionSize:
                self.experienceReplay.store(newState, actIndex, unavPenalty, newState, False) 
                self.earth.rewards.append([unavPenalty, sat.env.now])
                actIndex = random.randrange(self.actionSize)
                action = self.actions[actIndex]
                attempts += 1
            if linkedSats[action] is None:
                return -1, -1
        else:
            if noPingPong: # 保持 PingPong 防护...
                actIndex = -1
                if sat.upper == block.destination.linkedSat[1]: actIndex = 0
                elif sat.lower == block.destination.linkedSat[1]: actIndex = 1
                elif sat.right == block.destination.linkedSat[1]: actIndex = 2
                elif sat.left == block.destination.linkedSat[1]: actIndex = 3
            ...
            # =========== 关键修改：推理预测 ===========
            # newState 是 (feat_matrix, adj_matrix)，由于Keras需要batch维度，我们需要手动扩充维度
            feat_batch = np.expand_dims(newState[0], axis=0)
            adj_batch  = np.expand_dims(newState[1], axis=0)

            qValues = self.qNetwork.predict([feat_batch, adj_batch], verbose=0)
            for idx, direction in enumerate(self.actions):
                if linkedSats[direction] is None:
                    qValues[0][idx] = -np.inf

            if np.all(np.isneginf(qValues[0])):
                return -1, -1

            actIndex = int(np.argmax(qValues[0]))
            action   = self.actions[actIndex]
...
class ExperienceReplay:
    ...
    def getArraysFromBatch(self, batch):
        '''
        将 batch 拆分为 GAT 支持的多输入格式
        '''
        # state[0] 是节点特征, state[1] 是邻接矩阵
        states_feat = np.array([x[0][0] for x in batch])
        states_adj  = np.array([x[0][1] for x in batch])
        
        actions = np.array([x[1] for x in batch])
        rewards = np.array([x[2] for x in batch])
        
        next_st_feat = np.array([x[3][0] for x in batch])
        next_st_adj  = np.array([x[3][1] for x in batch])
        
        dones   = np.array([x[4] for x in batch])
        
        # 将分离的特征和邻接矩阵组装成列表，Keras 多输入模型需要这种格式
        states = [states_feat, states_adj]
        next_st = [next_st_feat, next_st_adj]
...
def getDeepStateDiff(block, sat, linkedSats):
    def normalize_angle_diff(angle_diff):
        # 确保角度差值在 [-180, 180] 之间，处理跨越 180 度的问题
        return (angle_diff + 180) % 360 - 180

    def normalize_queue_score(queue_length):
        if not np.isfinite(queue_length):
            return 1.0
        return float(np.clip(getDeepSatScore(queue_length) / max(queueVals, 1), 0.0, 1.0))
    ...
    def get_relative_position(neighbor_sat, current_coord, is_lat=True):
        try:
            ...
            return 0.0  # GAT中，如果节点不存在，特征填0

    def get_absolute_position(coord, is_lat=True):
        coord_deg = math.degrees(coord)
        if is_lat:
            return float(np.clip(coord_deg / 90.0, -1.0, 1.0))
        return float(np.clip(coord_deg / 180.0, -1.0, 1.0))
    ...
    # 获取队列状态（如果邻居不存在，则给无穷大，后续特征会自动被掩码掉）
    queuesU = getQueues(linkedSats['U'], DDQN=True) if linkedSats['U'] else {'U': np.inf, 'D': np.inf, 'R': np.inf, 'L': np.inf}
    queuesD = getQueues(linkedSats['D'], DDQN=True) if linkedSats['D'] else {'U': np.inf, 'D': np.inf, 'R': np.inf, 'L': np.inf}
    queuesR = getQueues(linkedSats['R'], DDQN=True) if linkedSats['R'] else {'U': np.inf, 'D': np.inf, 'R': np.inf, 'L': np.inf}
    queuesL = getQueues(linkedSats['L'], DDQN=True) if linkedSats['L'] else {'U': np.inf, 'D': np.inf, 'R': np.inf, 'L': np.inf}
    ...
    # 初始化 5x6 的特征矩阵和 5x5 的邻接矩阵
    node_features = np.zeros((5, 6), dtype=np.float32)
    adj_matrix = np.zeros((5, 5), dtype=np.float32)
    np.fill_diagonal(adj_matrix, 1.0)

    # ================= 节点 0: 自己 =================
    abs_lat = get_absolute_position(current_lat, is_lat=True)
    abs_lon = get_absolute_position(current_lon, is_lat=False)
    dest_rel_lat = get_relative_position(satDest, current_lat, is_lat=True)
    dest_rel_lon = get_relative_position(satDest, current_lon, is_lat=False)
    
    # 自己的6个特征：[绝对纬度, 绝对经度, 目标相对纬度, 目标相对经度, 补充特征0.0, 补充特征1.0(可用标志)]
    node_features[0] = [abs_lat, abs_lon, dest_rel_lat, dest_rel_lon, 0.0, 1.0]
    adj_matrix[0, 0] = 1.0  # 自己和自己相连 (自环)

    # ================= 节点 1-4: 邻居节点 =================
    directions = ['U', 'D', 'R', 'L']
    for idx, d in enumerate(directions, start=1):
        neighbor = linkedSats[d]
        if neighbor is not None:
            # 提取队列的 4 个得分
            qU = normalize_queue_score(queues_map[d]['U'])
            qD = normalize_queue_score(queues_map[d]['D'])
            qR = normalize_queue_score(queues_map[d]['R'])
            qL = normalize_queue_score(queues_map[d]['L'])
            
            # 提取 2 个相对位置
            rel_lat = get_relative_position(neighbor, current_lat, is_lat=True)
            rel_lon = get_relative_position(neighbor, current_lon, is_lat=False)

            # 邻居的6个特征：[U得分, D得分, R得分, L得分, 相对纬度, 相对经度]
            node_features[idx] = [qU, qD, qR, qL, rel_lat, rel_lon]
            
            # 邻接矩阵添加边: 自己(0)与邻居(idx)双向连接
            adj_matrix[0, idx] = 1.0
            adj_matrix[idx, 0] = 1.0
        else:
            # 缺失邻居使用高拥塞哨兵值，避免“全零”被模型误解为优质节点。
            node_features[idx] = [1.0, 1.0, 1.0, 1.0, 0.0, 0.0]

    # 返回供 GAT 消费的元组：(特征矩阵, 邻接矩阵)
...
def normalize_abs_position(sat):
    lat = float(np.clip(math.degrees(sat.latitude) / 90.0, -1.0, 1.0))
    lon = float(np.clip(math.degrees(sat.longitude) / 180.0, -1.0, 1.0))
    return lat, lon


def normalize_relative_position(from_sat, to_sat):
    if from_sat is None or to_sat is None:
        return 0.0, 0.0

    from_lat = math.degrees(from_sat.latitude)
    from_lon = math.degrees(from_sat.longitude)
    to_lat = math.degrees(to_sat.latitude)
    to_lon = math.degrees(to_sat.longitude)

    rel_lat = float(np.clip((to_lat - from_lat) / 180.0, -1.0, 1.0))
    rel_lon = float(np.clip(normalize_angle_diff(to_lon - from_lon) / 180.0, -1.0, 1.0))
...
def get_local_queue_features(node_sat):
    if node_sat is None:
        return [0.0, 0.0, 0.0, 0.0]

    queues = getQueues(node_sat, DDQN=True)
    return [
        normalize_queue_score(queues['U']),
        normalize_queue_score(queues['D']),
        normalize_queue_score(queues['R']),
        normalize_queue_score(queues['L']),
    ]


def get_role_one_hot(role_name):
    role = np.zeros(5, dtype=np.float32)
    role[ROLE_TO_INDEX[role_name]] = 1.0
    return role


def get_prev_satellite_id(block):
    if len(block.QPath) > 2:
        prev_id = block.QPath[-2][0]
        if isinstance(prev_id, str) and prev_id and prev_id[0].isdigit():
            return prev_id
    return None


def get_visited_satellite_ids(block):
    visited = set()
    for step in block.QPath[:-1]:
        node_id = step[0]
        if isinstance(node_id, str) and node_id and node_id[0].isdigit():
            visited.add(node_id)
    return visited


def build_sat_node_features(node_sat, role_name, sat_dest, block, prev_sat_id, visited_sat_ids):
    feat = np.zeros(NODE_FEATURE_DIM, dtype=np.float32)

    # 0: valid
    feat[0] = 1.0 if node_sat is not None else 0.0

    # 1:6 role one-hot
    feat[1:6] = get_role_one_hot(role_name)

    if node_sat is None:
        return feat

    # 6:8 absolute position
    abs_lat, abs_lon = normalize_abs_position(node_sat)
    feat[6] = abs_lat
    feat[7] = abs_lon

    # 8:10 relative position to destination-linked satellite
    rel_lat, rel_lon = normalize_relative_position(node_sat, sat_dest)
    feat[8] = rel_lat
    feat[9] = rel_lon

    # 10:14 local queue summary
    feat[10:14] = get_local_queue_features(node_sat)

    # 14: linked to any gateway
    feat[14] = 1.0 if node_sat.linkedGT is not None else 0.0

    # 15: linked gateway is the destination gateway
    feat[15] = 1.0 if (
        node_sat.linkedGT is not None and
        node_sat.linkedGT.ID == block.destination.ID
    ) else 0.0

    # 16: this node is previous hop
    feat[16] = 1.0 if (prev_sat_id is not None and node_sat.ID == prev_sat_id) else 0.0

    # 17: this node has been visited before
    feat[17] = 1.0 if node_sat.ID in visited_sat_ids else 0.0

    return feat
def getDeepStateHomogeneous(block, sat, linkedSats):
    sat_dest = block.destination.linkedSat[1]
    if sat_dest is None:
        print(f'{block.destination} has no linked satellite :(')
        return None

    prev_sat_id = get_prev_satellite_id(block)
    visited_sat_ids = get_visited_satellite_ids(block)
    ...
    node_features = np.stack([
        build_sat_node_features(
            node_sat=node_sat,
            role_name=role_name,
            sat_dest=sat_dest,
            block=block,
            prev_sat_id=prev_sat_id,
            visited_sat_ids=visited_sat_ids
        )
        for role_name, node_sat in node_specs
    ], axis=0).astype(np.float32)

    adj_matrix = np.eye(5, dtype=np.float32)
    for idx, (_, node_sat) in enumerate(node_specs[1:], start=1):
        if node_sat is not None:
            adj_matrix[0, idx] = 1.0
            adj_matrix[idx, 0] = 1.0
...
def getDeepStateDiffLastHop(block, sat, linkedSats):
    ...
    def encode_last_hop(last_hop_idx):
        # Cyclic encoding avoids introducing fake ordinal relation between directions.
        if last_hop_idx < 0:
            return 0.0, 0.0
        angle = (last_hop_idx % 4) * (math.pi / 2.0)
        return math.sin(angle), math.cos(angle)

    def get_last_satellite(block, sat):
        '''返回上一跳信息索引：-1(无/断连), 0(上), 1(下), 2(右), 3(左)'''
        actIndex = -1
        try:
            if len(block.QPath) > 2:
                if sat.upper and sat.upper.ID == block.QPath[-2][0]:
    ...
    queues_map = {'U': queuesU, 'D': queuesD, 'R': queuesR, 'L': queuesL}
    ...
    # 获取上一跳信息
    last_hop_idx = get_last_satellite(block, sat)
    last_hop_sin, last_hop_cos = encode_last_hop(last_hop_idx)
    
    # 自己的6个特征：[绝对纬度, 绝对经度, 目标相对纬度, 目标相对经度, last_hop_sin, last_hop_cos]
    node_features[0] = [abs_lat, abs_lon, dest_rel_lat, dest_rel_lon, last_hop_sin, last_hop_cos]
...
def saveDeepNetworks(outputPath, earth):
    ...
    if not onlinePhase:
        earth.DDQNA.qNetwork.save(outputPath + 'qNetwork_'+ str(len(earth.gateways)) + 'GTs' + '.keras')
        if ddqn:
            earth.DDQNA.qTarget.save(outputPath + 'qTarget_'+ str(len(earth.gateways)) + 'GTs' + '.keras')
    else:
        for plane in earth.LEO:
            for sat in plane.sats:
                sat.DDQNA.qNetwork.save(outputPath + sat.ID + 'qNetwork_'+ str(len(earth.gateways)) + 'GTs' + '.keras')
                if ddqn:
                    sat.DDQNA.qTarget.save(outputPath + sat.ID + 'qTarget_'+ str(len(earth.gateways)) + 'GTs' + '.keras')
