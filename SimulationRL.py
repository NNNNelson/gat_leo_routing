...
try:
    from spektral.layers import GAT, GlobalAvgPool
    from spektral.layers.ops import transpose
    SPEKTRAL_AVAILABLE = True
except ImportError:
    print("Warning: spektral not installed. Install with: pip install spektral")
...
# @profile
class DDQNAgent:
    def __init__(self, NGT, hyperparams, earth, sat_ID=None, use_gat=False):   
    ...
    def getNextHop(self, newState, linkedSats, sat, block):
        ...
        # highest value (Exploitation)
            ...
            if self.use_gat:
                ...
                # MLP模型预测
                qValues = self.qNetwork(newState).numpy()
