...
class Logger(object):
    def __init__(self, filename='logfile.log'):
        ...
        atexit.register(self.close)  # 注册 close 方法，以便在程序退出时调用
...
from keras.layers import Dense, Embedding, Reshape, Input, Conv2D, Flatten, GlobalAveragePooling1D, LayerNormalization, Dropout
from collections import deque

# GAT (Graph Attention Network) support
try:
    from spektral.layers import GATConv, GlobalAvgPool
    from spektral.layers.ops import transpose
    SPEKTRAL_AVAILABLE = True
    print("Spektral loaded successfully")
except ImportError as e:
    print(f"Warning: spektral import failed: {e}")
    SPEKTRAL_AVAILABLE = False

# 强制 TensorFlow 使用 GPU - 在这种情况下不值得使用 GPU，因为强化学习训练是每步进行的小缓冲区
...
USE_GAT = True  # Set to True to use GAT instead of MLP for Q-Network
...
class edge:
    def  __init__(self, sati, satj, slant_range, dji, dij, shannonRate):
        ...
        dji 和 dij 已被弃用。我们不再使用它们来判断哪个邻居处于左侧或右侧方向，而是改用它们的坐标（进行判断）。
该逻辑目前仅用于马尔可夫匹配（Markovian matching）过程中。
        '''
        self.i = sati   # sati ID
        self.j = satj   # satj ID
...
class DataBlock:
    ...
    本程序并非模拟来自每个用户的单个数据包，而是在网关（GT）处将数据汇聚成“块”——每个目的网关对应一个数据块。一旦某个数据块填满了数据，它就会作为一个整体单元发送至目的网关。
    """
...
class Earth:
    ...
    def updateSatelliteProcessesRL(self, graph):
        ...
        This function differentiates from the simple version by allowing continued operation of send-processes after
        constellation movement if the link is not broken.
    更新说明：该函数现已可以运行。之前的问题在于，当拓扑图更新且原有链路消失时，所有正在队列中等待发送的**轨道间（Inter-plane）**数据包都会被丢弃。
    （注：此处有一条矛盾的调试记录） 该函数运行仍不正确！ 当卫星移动距离过大导致必须形成新链路时，重新创建进程和队列的操作会失败。
    该函数考虑到了路径可能并不完整，且下一跳可能尚未选定的情况。
    函数功能：确保在星座移动后，所有卫星上的所有进程都能得到同步更新。具体步骤如下：
    路径更新：对所有正在等待发送或正在发送中的数据块（Blocks）更新其路由路径。
    ISL（星间链路）更新：更新指向新轨道间邻居卫星的引用（轨道内链路保持不变）。
    包括在 ISL 改变时更新缓冲区。
    包括在 ISL 改变时重新创建发送进程。
    尽管轨道内链路不变，但轨道内缓冲区中的数据块可能仍需移动。
    GSL（星地链路）更新：
    根据新状态（卫星当前是否有 GSL）和旧状态（卫星之前是否有 GSL）来相应地处理 GSL 缓冲区和进程。
    数据重分配：对于所有当前未在传输中（无论是发往卫星还是地面站）、且其对应的 ISL 或 GSL 依然存在的旧数据块，将根据它们到达卫星的先后时间，重新分配到对应的发送缓冲区中。
        """
        # update linked sats
        sats = []
        for plane in self.LEO:
    ...
    def moveConstellation(self, env, deltaT=3600, getRates = False):
        ...
        本函数根据地球自转移动星座，并同步移动星座内的各颗卫星。运动计算基于自上次星座移动以来所经过的时间，该时间由变量 deltaT 定义
        ...
        在卫星完成移动后，程序将执行所有链路（包括 GSL 地星链路 和 ISL 星间链路）的重连操作。在此过程中，所有数据块（Blocks）的传输路径
        将由于位置变化而重新生成；如有必要，数据块将被移动至正确的缓冲区中；同时，系统还会检查所有管理发送缓冲区的进程，
        以确保它们在新的网络状态下仍能正常工作。
        """

        # Get the data rate for a intra plane ISL - used for testing 获取轨道面内（intra-plane）星间链路（ISL）的数据速率 —— 用于测试。
        if getRates:
...
# @profile
class DDQNAgent:
    def __init__(self, NGT, hyperparams, earth, sat_ID=None, use_gat=True):   
        self.actions        = ('U', 'D', 'R', 'L')
        self.use_gat        = use_gat and SPEKTRAL_AVAILABLE
        ...
        if not self.importQ:
            ...
            if self.use_gat:
                self.qNetwork = self.createGATModel()
                if sat_ID is None:
                    print('----------------------------------')
                    print(f"GAT Q-NETWORK created (Graph Attention Network):")
                    print('----------------------------------')
                    self.qNetwork.summary()
                else:
                    print(f'Satellite {sat_ID} GAT Q-Network initialized')
                if ddqn:
                    self.qTarget = self.createGATModel()
                    if sat_ID is None:
                        print('----------------------------------')
                        print("DDQN enabled, GAT TARGET NETWORK created:")
                        print('----------------------------------')
                        self.qTarget.summary()
                    else:
                        print(f'Satellite {sat_ID} GAT Q-Target initialized')
            else:
    ...
    def getNextHop(self, newState, linkedSats, sat, block):
        ...
        # highest value (Exploitation)
            ...
            if self.use_gat:
                # GAT模型预测
                node_features, adjacency = newState
                # 添加批次维度
                node_features = np.expand_dims(node_features, axis=0)
                adjacency = np.expand_dims(adjacency, axis=0)
                qValues = self.qNetwork.predict([node_features, adjacency], verbose=0)
            else:
                # MLP模型预测
                # newState is a 1D vector, need to add batch dimension
                newState_batch = np.expand_dims(newState, axis=0)
                qValues = self.qNetwork.predict(newState_batch, verbose=0)
            
            # 屏蔽不可用的动作
            for i, direction in enumerate(self.actions):
                if linkedSats[direction] is None:
                    qValues[0][i] = -np.inf
            
            actIndex = np.argmax(qValues[0])
            action   = self.actions[actIndex]
    ...
    def makeDeepAction(self, block, sat, g, earth, prevSat=None):
       ...
       4. Receive reward/penalty
        ...
        if self.use_gat:
            # GAT方式：获取图状态
            node_features, adjacency = getGraphStateForGAT(block, sat, linkedSats, g, earth)
            if node_features is None:
                earth.lostBlocks += 1
                return 0
            newState = (node_features, adjacency)
        else:
            # MLP方式：获取向量状态（原有方式）
            if reducedState:
    ...
    def createGATModel(self):
        """
        创建图注意力网络（GAT）模型用于路由决策
        
        输入：
        - node_features: [batch_size, num_nodes, 7] 节点特征
        - adjacency: [batch_size, num_nodes, num_nodes] 邻接矩阵
        
        输出：
        - Q值: [batch_size, 4] 每个动作的Q值
        """
        if not SPEKTRAL_AVAILABLE:
            raise ImportError("spektral library is required for GAT. Install with: pip install spektral")
        
        # 定义输入层
        # 节点特征输入：[batch, nodes, features]
        node_features_input = Input(shape=(None, 7), name='node_features', dtype=tf.float32)
        # 邻接矩阵输入：[batch, nodes, nodes]
        adjacency_input = Input(shape=(None, None), sparse=False, name='adjacency', dtype=tf.float32)
        
        # GAT层1：8个注意力头，每个头32维 = 256维
        x = GATConv(
            channels=32,
            attn_heads=8,
            concat_heads=True,
        ...
        # 全局平均池化：将所有节点特征平均为单个向量
        x = GlobalAvgPool(name='global_pool')(x)  # [64] from all nodes
        ...
        # 输出层：4个动作的Q值
        q_values = Dense(self.actionSize, activation='linear', name='q_output')(x)
        
        # 构建模型
        model = Model(
            inputs=[node_features_input, adjacency_input],
            outputs=q_values,
            name='gat_q_network'
        )
    ...
    def train(self, sat, earth):
        ...
        if self.use_gat:
            # GAT методе处理：分离节点特征和邻接矩阵
            batch_node_features = []
            batch_adjacency = []
            batch_next_node_features = []
            batch_next_adjacency = []
            
            for state in states:
                if isinstance(state, tuple):
                    node_feat, adj = state
                    batch_node_features.append(node_feat)
                    batch_adjacency.append(adj)
            
            for next_state in nextStates:
                if isinstance(next_state, tuple):
                    node_feat, adj = next_state
                    batch_next_node_features.append(node_feat)
                    batch_next_adjacency.append(adj)
            
            # 转换为numpy数组并添加批次维度
            batch_node_features = np.array(batch_node_features, dtype=np.float32)
            batch_adjacency = np.array(batch_adjacency, dtype=np.float32)
            batch_next_node_features = np.array(batch_next_node_features, dtype=np.float32)
            batch_next_adjacency = np.array(batch_next_adjacency, dtype=np.float32)
            
            # 2. Compute expected reward using GAT
            if ddqn:
                futureRewards = self.qTarget.predict(
                    [batch_next_node_features, batch_next_adjacency], 
                    verbose=0
                )
            else:
                futureRewards = self.qNetwork.predict(
                    [batch_next_node_features, batch_next_adjacency], 
                    verbose=0
                )
            ...
            # 5. Train the model
            loss = self.qNetwork.fit(
                [batch_node_features, batch_adjacency],
                acts * expectedRewards[:, None],
        ...
        if ddqn and not self.hardUpd:
            self._soft_update_target_network()

    def _soft_update_target_network(self):
        """软更新：逐渐混合参数"""
        for target_weight, qnet_weight in zip(self.qTarget.trainable_weights, self.qNetwork.trainable_weights):
            target_weight.assign(self.tau * qnet_weight + (1.0 - self.tau) * target_weight)
...
class ExperienceReplay:
    ...
    def getArraysFromBatch(self, batch):
        ...
        Handles both GAT and MLP state formats:
        - GAT: state is a tuple (node_features, adjacency)
        - MLP: state is a vector
        '''
        # Check if we're dealing with GAT states (tuple) or MLP states (vector)
        first_state = batch[0][0]
        is_gat_format = isinstance(first_state, tuple)
        
        if is_gat_format:
            # GAT format: states are tuples of (node_features, adjacency)
            # Return them as-is for processing in train() function
            states  = [x[0] for x in batch]
            actions = np.array([x[1] for x in batch])
            rewards = np.array([x[2] for x in batch])
            next_st = [x[3] for x in batch]
            dones   = np.array([x[4] for x in batch])
            
            return states, actions, rewards, next_st, dones
        else:
            # MLP format: states are vectors
...
def initialize(env, popMapLocation, GTLocation, distance, inputParams, movementTime, totalLocations, outputPath, matching='Greedy'):
    ...
    if pathing == 'Deep Q-Learning':
        if not onlinePhase:
            ...
            earth.DDQNA = DDQNAgent(len(earth.gateways), hyperparams, earth, use_gat=USE_GAT)
        else:
            print('----------------------------------')
            ...
            for plane in earth.LEO:
                for sat in plane.sats:
                    sat.DDQNA = DDQNAgent(len(earth.gateways), hyperparams, earth, sat.ID, use_gat=USE_GAT)
            print('----------------------------------')
...
def getGraphStateForGAT(block, sat, linkedSats, g, earth):
    """
    将状态编码为图的节点特征 + 邻接矩阵，用于GAT模型
    ...
    返回：
    - node_features: [num_nodes, 7]特征向量
    ...
    # 初始化节点特征矩阵 [num_nodes, 7]
    node_features = []
    
    for i, (sat_node, node_type) in enumerate(nodes):
        if sat_node is None:
            ...
            try:
                ...
                if node_type == 'DEST':
                    ...
                    queues_node = getQueues(sat_node, DDQN=True)
                    queue_depth = getDeepSatScore(queues_node.get('U', []))
                
                # 相对位置（相对目标）
                rel_lat = get_relative_position(sat_node, current_lat, is_lat=True)
                rel_lon = get_relative_position(sat_node, current_lon, is_lat=False)
                
                # 绝对位置
                abs_lat = get_absolute_position(sat_node.latitude if hasattr(sat_node, 'latitude') else 0, latBias, coordGran)
                abs_lon = get_absolute_position(sat_node.longitude if hasattr(sat_node, 'longitude') else 0, lonBias, coordGran)
                
                # 节点类型编码
                type_encoding = {'U': 0, 'D': 0, 'R': 0, 'L': 0, 'SELF': 1, 'DEST': 2}[node_type]
                
                # 度数信息
                if node_type in ['U', 'D', 'R', 'L']:
                    try:
                        linked = getDeepLinkedSats(sat_node, g, earth)
                        degree = len([x for x in linked.values() if x is not None])
                    except:
                        degree = 0
                else:
                    degree = 0
                ...
                print(f"Error computing node features for node {i}: {e}")
                features = np.zeros(7, dtype=np.float32)
    ...
    # 构建邻接矩阵 [num_nodes, num_nodes]
    adjacency_matrix = np.zeros((num_nodes, num_nodes), dtype=np.float32)
    
    # 规则1：当前节点连接到所有邻接节点和目标
    current_idx = 4
    if nodes[0][0] is not None:  # U
        adjacency_matrix[current_idx, 0] = 1.0
    if nodes[1][0] is not None:  # D
        adjacency_matrix[current_idx, 1] = 1.0
    if nodes[2][0] is not None:  # R
        adjacency_matrix[current_idx, 2] = 1.0
    if nodes[3][0] is not None:  # L
        adjacency_matrix[current_idx, 3] = 1.0
    adjacency_matrix[current_idx, 5] = 1.0  # 连接到目标卫星
    
    # 规则2：对称化邻接矩阵（无向图）
    adjacency_matrix = adjacency_matrix + adjacency_matrix.T
    
    # 规则3：邻接卫星之间的链接
    neighbors = [nodes[0][0], nodes[1][0], nodes[2][0], nodes[3][0]]
    for i, neigh_i in enumerate(neighbors):
        if neigh_i is None:
            continue
        for j, neigh_j in enumerate(neighbors):
            if neigh_j is None or i >= j:
                continue
            try:
                if g.has_edge(neigh_i.ID, neigh_j.ID):
...
def plotLatenciesBars(percentages, outputPath):
    ...
    柱状图，其中每根柱子代表一个具有不同网关数量的场景，每种颜色代表三种延迟中的一种。  # 堆叠柱状图（Stacked Bar Chart）
    '''
...
def plotQueues(queues, outputPath, GTnumber):
    ...
    Queues each package has faced：指一个数据包从源节点到目的节点，在经过每一个中间路由节点时所遇到的排队情况。
    因为 cumulative=True 被激活了，所以画出来的是 CDF。如果你也想画 PDF（看哪个队列长度最常见），你需要再写一行代码，并将 cumulative 设置为 False
    ''' 
