...
class DDQNAgent:
    ...
    def train(self, sat, earth):
        ...
        # 2. Compute expected reward (注意这里 nextStates 已经是一个列表 [features, adjs])
        if ddqn:
            futureRewards = self.qTarget(nextStates)
        else:
    ...
    def getNextHop(self, newState, linkedSats, sat, block):
        ...
        if explore and random.uniform(0, 1)<self.alignEpsilon(self.step, sat):
            ...
            qValues = self.qNetwork([feat_batch, adj_batch]).numpy()
            actIndex = np.argmax(qValues)
            action   = self.actions[actIndex]
...
def getDeepStateDiff(block, sat, linkedSats):
    ...
    # ================= 节点 0: 自己 =================
    abs_lat = get_absolute_position(current_lat, latBias, coordGran)
    abs_lon = get_absolute_position(current_lon, lonBias, coordGran)
    ...
    for idx, d in enumerate(directions, start=1):
        ...
        if neighbor is not None:
            # 提取队列的 4 个得分
            qU = getDeepSatScore(queues_map[d]['U'])
            qD = getDeepSatScore(queues_map[d]['D'])
            qR = getDeepSatScore(queues_map[d]['R'])
            qL = getDeepSatScore(queues_map[d]['L'])
            ...
            # 邻接矩阵添加边: 自己(0)与邻居(idx)双向连接，并加上邻居的自环
            adj_matrix[0, idx] = 1.0
            adj_matrix[idx, 0] = 1.0
            adj_matrix[idx, idx] = 1.0
        else:
            # 如果没有该方向的邻居，该行的特征保持为 0，并且邻接矩阵对应行和列保持为 0
            pass
...
def getDeepStateDiffLastHop(block, sat, linkedSats):
    ...
    # 自己的6个特征：[绝对纬度, 绝对经度, 目标相对纬度, 目标相对经度, 上一跳索引(代替了0.0), 可用标志]
    node_features[0] = [abs_lat, abs_lon, dest_rel_lat, dest_rel_lon, float(last_hop_idx), 1.0]
    adj_matrix[0, 0] = 1.0  
